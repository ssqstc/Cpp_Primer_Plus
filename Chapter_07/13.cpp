 ...
 * 2. 指针：函数参数使用指针，传递结构体数据的地址。
 * 3. 数学运算：利用sqrt和atan2函数进行坐标转换。
 * 4. 函数：定义并调用函数实现坐标转换和显示。
 *
 * 注意点：
...
void rect_to_polar(const rect *pxy, polar *pda);  // 直角坐标转换为极坐标的函数声明
void show_polar(const polar *pda);                // 显示极坐标的函数声明
...
int main() {
    ...
    while (cin >> rplace.x >> rplace.y) {   // 输入循环，按q退出
        rect_to_polar(&rplace, &pplace);    // 调用函数转换坐标
